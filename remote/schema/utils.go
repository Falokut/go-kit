...
func SetProperties(field reflect.StructField, s *Schema) {
	...
	if ok {
		f := CustomGenerators.getGeneratorByName(customValue)
		if f != nil {
			f(field, s)
		}
	}
...
func setValidators(field reflect.StructField, s *Schema) {
	validators := getValidatorsMap(field)
	if validators == nil {
		return
	}
...
func setMax(s *Schema, v string) {
	...
	case "string":
		s.MaxLength = &value
	case "integer":
		s.Maximum = parseInt(v)
	case "array":
		s.MaxItems = &value
	}
}

func setMin(s *Schema, v string) {
	value, err := strconv.ParseUint(v, 10, 64)
	if err != nil {
		return
	}
	...
	case "string":
		s.MinLength = &value
	case "integer":
		s.Minimum = parseInt(v)
	case "array":
		s.MinItems = &value
	}
}

func setOneOf(s *Schema, v string) {
