...
func (c *Client) Run(ctx context.Context, eventHandler *EventHandler) error {
	...
	for {
		...
		sessionCtx := log.ToContext(ctx,
			log.String("sessionId", sessionId),
			log.String("configService", host),
		)
		...
		c.logger.Error(sessionCtx, errors.WithMessage(err, "run config service session"))
...
func (c *Client) notifyModuleReady(ctx context.Context, requirements ModuleRequirements) error {
	...
	declaration := BackendDeclaration{
		...
		Address:         c.moduleInfo.OuterAddress,
	}
